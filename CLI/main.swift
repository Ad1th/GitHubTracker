...
enum ANSI {
    ...
    // Heatmap level colors (256-color green scale)
    static func levelColor(_ level: ContributionDay.IntensityLevel) -> String {
        switch level {
...
struct GHTrackerCLI {
    ...
    static func renderTerminalDashboard(data: ContributionData) {
        ...
        print("\(ANSI.bold)  GITHUB CONTRIBUTION TRACKER\(ANSI.reset)                       \(ANSI.cyan)@\(u)\(ANSI.reset)")
        ...
        // Legend
        print("\n  \(ANSI.dim)Legend: \(ANSI.levelColor(.zero))■\(ANSI.reset) 0  \(ANSI.levelColor(.low))■\(ANSI.reset) 1-3  \(ANSI.levelColor(.medium))■\(ANSI.reset) 4-7  \(ANSI.levelColor(.high))■\(ANSI.reset) 8-11  \(ANSI.levelColor(.max))■\(ANSI.reset) 12+\(ANSI.reset)")
        
        // Language Statistics
        if !data.languageStats.isEmpty {
            print("\n  \(ANSI.bold)TOP LANGUAGES\(ANSI.reset)")
            for lang in data.languageStats.prefix(5) {
                let barLength = Int(lang.percentage / 4.0) // 25 chars max
        ...
        for row in 0..<7 {
            ...
            for w in 0..<weeks.count {
                if let day = weeks[w][row] {
                    let color = ANSI.levelColor(day.level)
                    if day.isToday {
                    ...
                    line += "\(ANSI.levelColor(.zero))·\(ANSI.reset)"
                }
                line += " "
            }
            print(line)
