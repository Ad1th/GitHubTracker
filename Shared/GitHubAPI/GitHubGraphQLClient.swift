...
public final class GitHubGraphQLClient {
    ...
    public func fetchContributionData(username: String, token: String) async throws -> ContributionData {
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw GitHubAPIError.networkError("Invalid response from server")
        }
    ...
    private func processGraphQLUser(_ user: GraphQLUser, username: String) -> ContributionData {
        ...
        // Backward calculation for current streak
        for day in sortedDays.reversed() {
            if day.count > 0 {
                currentStreak += 1
            } else if day.isToday {
                // If 0 today, don't break streak if yesterday was active
                continue
            } else {
                break
            }
...
// MARK: - GraphQL Decoding Structures
